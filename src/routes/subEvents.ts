import {
  ...
  getInsumesBySubEventIdController,
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (
        ...
        if (
          !databaseRegex.subEvents.id_global_event.test(
            req.params.id_global_event!.toString()
          )
        ) {
          return res.status(400).json({
...
SubEventsRouter.get(
  "/get/insumes/:id_sub_event",
  async (req: Request, res: Response) => {
    try {
      return await getInsumesBySubEventIdController(req, res);
    } catch (err) {
      return res
        .status(500)
        .send(JSON.stringify({ message: "Error interno del servidor :(" }));
